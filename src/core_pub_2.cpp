...
bool calcOdometry(double diff_time)
{
 ...
 wheel_l = (double)last_diff_tick[LEFT]*sampling*PI/180.0;
 wheel_r = -(double)last_diff_tick[RIGHT]*sampling*PI/180.0;
//  ROS_INFO("%f %f",wheel_l,wheel_r);
  if (isnan(wheel_l))
    wheel_l = 0.0;
...
//  if((goal_velocity_from_cmd[ANGULAR] != 0.0)&&(goal_velocity_from_cmd[LINEAR]==0.0))
//  {delta_s = 0; theta_1 = 1;}
  /*
  else
  {theta_1 = odom_pose[2];}
*/ 
  ...
  if((goal_velocity_from_cmd[LINEAR] != 0.0))
  {
...
*/
/*
  if((goal_velocity_from_cmd[ANGULAR] != 0.0))
  {
  odom_pose[0] = last_odom_pose[0];
    odom_pose[1] = last_odom_pose[1];}
*/
//  if(goal_velocity_from_cmd[ANGULAR] == 0.0)
//  {  odom_pose[0] += delta_s * cos(odom_pose[2] + (delta_theta / 2.0));
//     odom_pose[1] += delta_s * sin(odom_pose[2] + (delta_theta / 2.0));}

  ROS_INFO("%f %f %f ",odom_pose[0], odom_pose[1],odom_pose[2]);
  //odom_pose[2] = theta;
