  ...
  isChecked = false;
    ...
    if (isChecked == false)
    {
      ...
      else
      {
        ...
        strcat(odom_child_frame_id, "/dummy");
...
float constrain(float input, float low, float high)//low,high,input으로 제약걸어줌
{
  if (input < low)
  {  input = low;}
  else if(input > high)
...
bool calcOdometry(double diff_time)
{
 ...
 wheel_r = (double)last_diff_tick[RIGHT];
  ...
  last_theta = theta;
  ROS_INFO("%f, %f a",theta,odom_pose[2]);
  return true;
}
